...
<Tip>
  To view commands for projects and libraries, open the Nx dependency graph with
  `bun nx graph`. Click on an app or library, then the top right button to view
  all its tasks.
</Tip>

<Note>
  OrbitKit uses Nx to run commands across apps & packages in the monorepo, you
  can also use `bun --filter` commands to do the same without dependant tasks
  running or caching.
</Note>
...
You can start up for example only the web application by using `bun nx run @orbitkit/web:dev`.
...
You can build for example only the web application by using `bun nx run @orbitkit/web:build`.
...
The `start` command starts up all of the apps in the monorepo in production mode. This is done after building all of the apps & packages automatically via Nx dependant tasks, so you don't need to build before you run the start command.
...
```bash
bun nx run @orbitkit/storybook:dev
```
...
```bash
bun nx run @orbitkit/storybook:build
```
...
```bash
bun nx run @orbitkit/db:generate
```
...
```bash
bun nx run @orbitkit/db:migrate
```
...
You can invoke these commands using `bun`, for example to check for collisions and inconsistencies between your database and the schema, you can run `bun --filter @orbitkit/db check`.
