...
- All other packages are compiled with `tsc` because they are used internally and we want to keep the TS LSP footprint low, we can use a combination of the awesome Nx filtering to minimise the `dev` watch processes running while development.
...
```bash
bun nx run-many --target=dev --projects=@orbitkit/web --exclude=packages-config --exclude=@orbitkit/ui
```
...
{
  "scripts": {
    "dev:web:api": "bun nx run-many --target=dev --projects=@orbitkit/web --exclude=packages-config --exclude=@orbitkit/ui"
  }
}
```
