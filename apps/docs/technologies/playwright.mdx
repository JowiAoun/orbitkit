...
- To run the tests, you use Nx and the `bun run test:e2e` command. Filtering to one or more applications is done using the `--project` or `-p` flag followed by the application(s) name such as `bun run test:e2e -p @orbitkit/web`.
- To provide more flags to playwright while running it, you can do so as usual. For example, `bun run test:e2e --headed` will run the tests in the web application in headed mode.
